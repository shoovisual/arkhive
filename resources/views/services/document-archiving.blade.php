...
<main class="w-full">
    ...
    <div id="service" class="max-w-7xl grid grid-cols-5 space-x-8 my-10 mx-auto px-8">
        ...
        <div class="service-list col-span-4">
            <ul class="text-xl flex flex-wrap font-[ubuntu] gap-y-2 text-white">
    ...
    <div id="service" class="max-w-7xl grid grid-cols-5 space-x-8 my-10 mx-auto px-8">
        ...
        <div class="service-list col-span-4">
            <ul class="text-xl grid grid-cols-1 md:grid-cols-2 gap-x-4 font-[ubuntu] gap-y-2 text-white">
...
<script>
    ...
    // Create the scroll trigger for repeated animation
    gsap.utils.toArray(".anim-section").forEach(section => {
    const children = section.querySelectorAll(".anim-child");
        // Create one ScrollTrigger per SECTION so we can reset when leaving
        ScrollTrigger.create({
            ...
            // Fired each time the section scrolls into view
            onEnter: () => {
            gsap.to(children, {
            opacity: 1,
            y: 0,
            ...
            // If this section contains counters run them too
            if (section.querySelector(".count-up")) runCounters(section);
            },

            // Optional: wipe styles so every replay starts from scratch
            onLeaveBack: () => {
            children.forEach(c => {
                gsap.set(c, { clearProps: "opacity,transform,filter" });
                // reset counters to zero so they replay cleanly
